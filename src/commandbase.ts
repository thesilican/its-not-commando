...
export abstract class CommandBase {
    ...
    protected constructor(options: CommandBaseOptions) {
        ...
        this.subcommands = options.subcommands?.map(s => new s().setParent(this)) ?? undefined;
    ...
    public execute(commandText: string, msg: CommandMessage, client: Client): boolean {
        ...
        if (this.subcommands) {
            ...
            for (const sub of this.subcommands) {
                if (args.length > 0 && (args[0] === sub.name || sub.aliases.includes(args[0]))) {
            ...
            if (result !== null) {
                ...
                let messageText = "The correct usage is `" + client.prefix + this.fullName() + " " + this.usage.toString() + "`\n\n";
                ...
                messageText += "(Use `" + client.prefix + "help " + this.fullName() + "` for more details)";

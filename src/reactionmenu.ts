...
export class ReactionMenu {
  ...
  constructor(
    ...
    emojis: string[],
    onReaction: ReactionMenuCallback,
    options: ReactionMenuOptions | undefined
  ) {
  ...
  async start() {
    const filter = (reaction: MessageReaction, user: User) =>
      this.emojis.includes(reaction.emoji.name) && user.bot === false;
    ...
    while (loop) {
      try {
        let messageReactions = await this.message.awaitReactions(filter, {
          max: 1,
          time: this.timespan,
        });
        let reaction = messageReactions.first();

        let user = reaction.users

...
class ValidatorSubCommand extends SubCommand {
  constructor() {
    super({
      ...
      subcommands: [
        ChannelValidatorCommand,
        UserValidationCommand,
        NumberValidationCommand,
      ],
    });
  }
}
...
class PasswordCommand extends SubCommand {
  ...
  public async run(msg: CommandMessage) {
    let text = await msg.promptText(
      "Tell the the password right away",
      undefined,
      { seconds: 10 }
    );
    if (text === null) {
...
class TextMenuCommand extends SubCommand {
  ...
  public async run(msg: CommandMessage) {
    ...
    (await msg.say(getPage())).createMenu(
      ["👈", "👉"],
      async (reaction, msg) => {
...
class ReactionMenuCommand extends SubCommand {
  ...
  public async run(msg: CommandMessage) {
    (await msg.say("Options?")).createMenu(
      ["👍", "👎"],
      async () => {
